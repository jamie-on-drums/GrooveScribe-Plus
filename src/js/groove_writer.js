...
function GrooveWriter() {
  ...
  var class_time_division = parseInt(
    root.myGrooveUtils.getQueryVariableFromURL("Div", "16"),
    10
  ); // default to 16ths
  ...
  function addOrRemoveKeywordFromClass(tag_class, keyword, addElseRemove) {
    ...
    if (tag_class) {
      if (tag_class.className != undefined) {
        ...
        console.log(
          "Warning in addOrRemoveKeywordFromClassName: null className for tag id: " +
            tag_class.id
        );
        console.trace();
        return_val = false;
      ...
      console.log(
        "Warning in addOrRemoveKeywordFromClassName: null tag_class passed in"
      );
      return_val = false;
    }
  ...
  function addOrRemoveKeywordFromClassById(tagId, keyword, addElseRemove) {
    ...
    if (!addOrRemoveKeywordFromClass(tag_class, keyword, addElseRemove))
      console.log(
        "Warning in addOrRemoveKeywordFromClassById bad ID: " + tagId
      );
  }
  ...
  function get_snare_state(id, returnType) {
    ...
    if (
      document.getElementById("snare_flam" + id).style.color ==
      constant_note_on_color_rgb
    ) {
      if (returnType == "ABC") return constant_ABC_SN_Flam; // snare flam
      else if (returnType == "URL") return "f"; // snare flam
    }
    if (
      document.getElementById("snare_drag" + id).style.color ==
      constant_note_on_color_rgb
    ) {
      if (returnType == "ABC") return constant_ABC_SN_Drag; // snare drag
      else if (returnType == "URL") return "d"; // snare drag
    }
    if (
      document.getElementById("snare_ghost" + id).style.color ==
      constant_note_on_color_rgb
    ) {
      if (returnType == "ABC") return constant_ABC_SN_Ghost; // ghost note
    ...
    if (
      ...
      if (returnType == "ABC") return constant_ABC_SN_Accent; // snare accent
    ...
    if (
      ...
      if (returnType == "ABC") return constant_ABC_SN_Normal; // snare normal
      else if (returnType == "URL") return "o"; // snare normal
    }
    if (
      document.getElementById("snare_xstick" + id).style.color ==
      constant_note_on_color_rgb
    ) {
      if (returnType == "ABC") return constant_ABC_SN_XStick; // snare Xstick
      else if (returnType == "URL") return "x"; // snare xstick
    }
    if (
      document.getElementById("snare_buzz" + id).style.color ==
      constant_note_on_color_rgb
    ) {
      if (returnType == "ABC") return constant_ABC_SN_Buzz; // snare Buzz
      else if (returnType == "URL") return "b"; // snare Buzz
    }

    if (returnType == "ABC") return false; // off (rest)
  ...
  function get_tom_state(id, tom_num, returnType) {
    var tomOn =
      document.getElementById("tom_circle" + tom_num + "-" + id).style
        .backgroundColor == constant_note_on_color_rgb;
    ...
    switch (mode) {
      case "off":
        document.getElementById(
          "tom_circle" + tom_num + "-" + id
        ).style.backgroundColor = constant_note_off_color_hex;
        document.getElementById(
          "tom_circle" + tom_num + "-" + id
        ).style.borderColor = constant_note_border_color_hex;
        break;
      case "normal":
        document.getElementById(
          "tom_circle" + tom_num + "-" + id
        ).style.backgroundColor = constant_note_on_color_hex;
        document.getElementById(
          "tom_circle" + tom_num + "-" + id
        ).style.borderColor = constant_note_border_color_hex;
        if (make_sound)
          switch (tom_num) {
            ...
            default:
              console.log(
                "bad switch in set_tom_state. bad tom num:" + tom_num
              );
              break;
          }
        break;
  ...
  function get_kick_state(id, returnType) {
    var splashOn =
      document.getElementById("kick_splash" + id).style.color ==
      constant_note_on_color_rgb;
    var kickOn =
    ...
    if (splashOn && kickOn) {
      if (returnType == "ABC") return constant_ABC_KI_SandK; // kick & splash
      else if (returnType == "URL") return "X"; // kick & splash
    } else if (splashOn) {
      if (returnType == "ABC") return constant_ABC_KI_Splash; // splash only
      else if (returnType == "URL") return "x"; // splash only
    } else if (kickOn) {
      if (returnType == "ABC") return constant_ABC_KI_Normal; // kick normal
  ...
  function get_hh_state(id, returnType) {
    ...
    if (
      document.getElementById("hh_ride" + id).style.color ==
      constant_note_on_color_rgb
    ) {
      if (returnType == "ABC") return constant_ABC_HH_Ride; // ride
      else if (returnType == "URL") return "r"; // ride
    }
    if (
      document.getElementById("hh_ride_bell" + id).style.color ==
      constant_note_on_color_rgb
    ) {
      if (returnType == "ABC") return constant_ABC_HH_Ride_Bell; // ride bell
      else if (returnType == "URL") return "b"; // ride bell
    }
    if (
      document.getElementById("hh_cow_bell" + id).style.color ==
      constant_note_on_color_rgb
    ) {
      if (returnType == "ABC") return constant_ABC_HH_Cow_Bell; // cow bell
      else if (returnType == "URL") return "m"; // (more) cow bell
    }
    if (
      document.getElementById("hh_crash" + id).style.color ==
      constant_note_on_color_rgb
    ) {
      if (returnType == "ABC") return constant_ABC_HH_Crash; // crash
      else if (returnType == "URL") return "c"; // crash
    }
    if (
      document.getElementById("hh_stacker" + id).style.color ==
      constant_note_on_color_rgb
    ) {
      if (returnType == "ABC") return constant_ABC_HH_Stacker; // stacker
    ...
    if (
      ...
      if (returnType == "ABC") return constant_ABC_HH_Metronome_Normal; // beep
    ...
    if (
      ...
      if (returnType == "ABC") return constant_ABC_HH_Metronome_Accent; // beep
      else if (returnType == "URL") return "N"; // beep
    }
    if (
      document.getElementById("hh_open" + id).style.color ==
      constant_note_on_color_rgb
    ) {
      if (returnType == "ABC") return constant_ABC_HH_Open; // hh Open
      else if (returnType == "URL") return "o"; // hh Open
    }
    if (
      document.getElementById("hh_close" + id).style.color ==
      constant_note_on_color_rgb
    ) {
      if (returnType == "ABC") return constant_ABC_HH_Close; // hh close
      else if (returnType == "URL") return "+"; // hh close
    }
    if (
      document.getElementById("hh_accent" + id).style.color ==
      constant_note_on_color_rgb
    ) {
      if (returnType == "ABC") return constant_ABC_HH_Accent; // hh accent
      else if (returnType == "URL") return "X"; // hh accent
    }
    if (
      document.getElementById("hh_cross" + id).style.color ==
      constant_note_on_color_rgb
    ) {
      if (returnType == "ABC") return constant_ABC_HH_Normal; // hh normal
  ...
  function set_hh_state(id, mode, make_sound) {
    ...
    switch (mode) {
      case "off":
        document.getElementById("hh_cross" + id).style.color =
          constant_hihat_note_off_color_hex;
        break;
      case "normal":
        document.getElementById("hh_cross" + id).style.color =
          constant_note_on_color_hex;
  ...
  function set_sticking_state(id, new_state, make_sound) {
    ...
    switch (new_state) {
      ...
      case "count":
        var count_state =
          root.myGrooveUtils.figure_out_sticking_count_for_index(
            id,
        ...
        document.getElementById("sticking_count" + id).innerHTML =
          "" + count_state;
        break;
  ...
  function get_sticking_state(id, returnType) {
    ...
    if (both_ele.style.color == constant_sticking_both_on_color_rgb) {
      ...
      if (returnType == "ABC") return constant_ABC_STICK_OFF; // off (rest)
      else if (returnType == "URL") return "-"; // off (rest)
    }
  ...
  function hilight_individual_note(instrument, id) {
    ...
    if (
      ...
      if (
        class_cur_hh_highlight_id <
        class_notes_per_measure * class_number_of_measures
      )
        document.getElementById(
          "hi-hat" + class_cur_hh_highlight_id
    ...
    if (
      ...
      if (
        class_cur_tom1_highlight_id <
        class_notes_per_measure * class_number_of_measures
      )
        document.getElementById(
          "tom1-" + class_cur_tom4_highlight_id
        ).style.borderColor = "transparent";
      class_cur_tom1_highlight_id = false;
    }
    if (
      ...
      if (
        class_cur_tom4_highlight_id <
        class_notes_per_measure * class_number_of_measures
      )
        document.getElementById(
          "tom4-" + class_cur_tom4_highlight_id
        ).style.borderColor = "transparent";
      class_cur_tom4_highlight_id = false;
    }
    if (
      ...
      if (
        class_cur_snare_highlight_id <
        class_notes_per_measure * class_number_of_measures
      )
        document.getElementById(
          "snare" + class_cur_snare_highlight_id
        ).style.borderColor = "transparent";
      class_cur_snare_highlight_id = false;
    }
    if (
      ...
      if (
        class_cur_kick_highlight_id <
        class_notes_per_measure * class_number_of_measures
      )
        document.getElementById(
          "kick" + class_cur_kick_highlight_id
        ).style.borderColor = "transparent";
      class_cur_kick_highlight_id = false;
    }
  ...
  function hilight_all_notes_on_same_beat(instrument, id) {
    ...
    if (class_cur_all_notes_highlight_id !== false) {
      ...
      var bg_ele = document.getElementById(
        "bg-highlight" + class_cur_all_notes_highlight_id
      );
      if (bg_ele) {
    ...
    document.getElementById(
      "bg-highlight" + class_cur_all_notes_highlight_id
    ).style.background = "rgba(50, 126, 173, 0.2)";
  }
  ...
  function clear_all_highlights(instrument) {
    ...
    if (class_cur_tom1_highlight_id !== false) {
      document.getElementById(
        "tom1-" + class_cur_tom1_highlight_id
      ).style.borderColor = "transparent";
      class_cur_tom1_highlight_id = false;
    }
    ...
    if (contextMenu) {
      ...
      if (anchorPoint) {
        ...
        contextMenu.style.left =
          anchorPos.x + anchorPoint.offsetWidth - 150 + "px";
      }
  ...
  root.groovesAnchorClick = function (event) {
    ...
    if (contextMenu) {
      ...
      if (anchorPoint) {
        ...
        contextMenu.style.top = anchorPos.y + anchorPoint.offsetHeight + "px";
        contextMenu.style.left =
          anchorPos.x + anchorPoint.offsetWidth - 283 + "px";
      }
      root.myGrooveUtils.showContextMenu(contextMenu);
    }
  };
    ...
    switch (option_type) {
      case "Solo":
        ...
        if (!current) {
          ...
          addOrRemoveKeywordFromClassById(
            "metronomeOptionsContextMenuSolo",
            "menuChecked",
            true
          );
      ...
      case "OffTheOne":
        ...
        if (contextMenu) {
          var anchorPoint = document.getElementById(
            "metronomeOptionsContextMenuOffTheOne"
          );

          if (anchorPoint) {
            var anchorPos = getTagPosition(anchorPoint);
            contextMenu.style.top =
              anchorPos.y + anchorPoint.offsetHeight + "px";
            contextMenu.style.left =
              anchorPos.x + anchorPoint.offsetWidth - 150 + "px";
          }
          root.myGrooveUtils.showContextMenu(contextMenu);
  ...
  root.metronomeOptionsMenuOffsetClickPopupClick = function (option_type) {
    ...
    var myElements = document.querySelectorAll(
      ".metronomeOptionsOffsetClickContextMenuItem"
    );
    for (var i = 0; i < myElements.length; i++) {
  ...
  root.permutationPopupClick = function (perm_type) {
    ...
    switch (perm_type) {
      case "kick_16ths":
        ...
        document.getElementById("PermutationOptions").innerHTML =
          root.HTMLforPermutationOptions();
  ...
  root.noteLabelPopupClick = function (instrument, action) {
    ...
    var startIndex =
      class_notes_per_measure * (class_measure_for_note_label_click - 1);
    for (var i = startIndex; i - startIndex < class_notes_per_measure; i++) {
      if (action == "all_off") {
        ...
        var num_notes_per_count =
          class_time_division / class_note_value_per_measure;
        var cur_state = get_kick_state(i, "ABC");
        var kick_is_on = false;
        if (
          cur_state == constant_ABC_KI_SandK ||
          cur_state == constant_ABC_KI_Normal
        )
          kick_is_on = true;
        set_kick_state(
          i,
  ...
  function get_kick16th_triplets_permutation_array(section) {
    ...
    for (var index = 0; index < 48; index++) {
      switch (section) {
        ...
        case 15: // 1st Quad
          console.log(
            "bad case in get_kick16th_triplets_permutation_array_for_16ths()"
          );
          break;
  ...
  // create a new instance of an array with all the values prefilled with false
  // the array size is 64th notes for the current time signature
  // 4/4 would be 64 notes
  // 5/4 would be 80 notes
  // 2/4 would be 32 notes
  // 4/2 would be 64 notes
  function get_empty_note_array_in_32nds() {
    var notes_per_4_beats = 64;
    if (usingTriplets()) notes_per_4_beats = 48;
    var num_notes =
      (class_num_beats_per_measure * notes_per_4_beats) /
      class_note_value_per_measure;
  ...
  function shouldDisplayPermutationForSection(sectionNum) {
    ...
    switch (sectionNum) {
      ...
      default:
        console.log(
          "bad case in groove_writer.js:shouldDisplayPermutationForSection()"
        );
        return false;
      //break;
    }
  ...
  // query the clickable UI and generate a 64 element array representing the notes of one measure
  // note: the ui may have fewer notes, but we scale them to fit into the 64 elements proportionally
  // If using triplets returns 48 notes.   Otherwise always 64.
  //
  ...
  function get32NoteArrayFromClickableUI(
    ...
    for (var i = 0; i < class_notes_per_measure; i++) {
      ...
      if (isStickingsVisible())
        Sticking_Array[array_index] = get_sticking_state(
      ...
      if (isTomsVisible()) {
        Toms_Array[0][array_index] = get_tom_state(
          i + startIndexForClickableUI,
          1,
          "ABC"
        );
        Toms_Array[3][array_index] = get_tom_state(
          i + startIndexForClickableUI,
          4,
          "ABC"
        );
      ...
      Snare_Array[array_index] = get_snare_state(
        i + startIndexForClickableUI,
        "ABC"
      );

      Kick_Array[array_index] = get_kick_state(
        i + startIndexForClickableUI,
        "ABC"
      );
    }
  ...
  function filter_kick_array_for_permutation(old_kick_array) {
    ...
    for (var i in old_kick_array) {
      if (
        old_kick_array[i] == constant_ABC_KI_Splash ||
    ...
    switch (class_permutation_type) {
        ...
        for (i = 0; i < numSections; i++) {
          if (shouldDisplayPermutationForSection(i)) {
            if (
              ...
              new_snare_array =
                get_snare_accent_with_diddle_permutation_array(i);
            else if (
      ...
      default:
        if (class_time_division < 16)
          num_notes_for_swing =
            (8 * class_num_beats_per_measure) / class_note_value_per_measure;
        else
          num_notes_for_swing =
            (16 * class_num_beats_per_measure) / class_note_value_per_measure;
    ...
    if (root.myGrooveUtils.debugMode) {
      ...
      if (searchDataEle) {
        ...
        searchDataEle.innerHTML =
          '<p style="margin-left: 10px;"><b>' + searchURL + "</b><p>";
      }
    }
  };
      ...
      case "kick_16ths": // use the hh & snare from the user
        ...
        for (i = 0; i < numSections; i++) {
          if (shouldDisplayPermutationForSection(i)) {
            ...
            root.myGrooveUtils.note_mapping_array =
              root.myGrooveUtils.note_mapping_array.concat(
  ...
  function downloadImages(imageType) {
    ...
    for (var i = 0; i < svg_images.length - 1; i++) {
      ...
      myPablo.download(imageType, filename, function (result) {
        if (result.error) {
          alert(
            "An error occurred when trying to convert the sheet music to a PNG file."
          );
        }
      });
    }
  ...
  root.closeMeasureButtonClick = function (measureNum) {
    ...
    root.expandAuthoringViewWhenNecessary(
      class_notes_per_measure,
      class_number_of_measures
    );
  ...
  root.clearAllNotes = function () {
    for (
      var i = 0;
      i < class_number_of_measures * class_notes_per_measure;
      i++
    ) {
  ...
  root.showHideToms = function (force, showElseHide, dontRefreshScreen) {
    var OnElseOff = showHideCSS_ClassVisibility(
      ".toms-container",
      force,
      showElseHide
    );
  ...
  root.stickingsShowHide = function (force, showElseHide, dontRefreshScreen) {
    var OnElseOff = showHideCSS_ClassDisplay(
      ".stickings-container",
      force,
      showElseHide,
      "block"
  ...
  root.printMusic = function () {
    ...
    if (
      root.browserInfo.browser == "Chrome" &&
  ...
  root.runsOnPageLoad = function () {
    ...
    selectButton(
      document.getElementById("subdivision_" + class_notes_per_measure + "ths")
    );
    ...
    root.myGrooveUtils.midiEventCallbacks.loadMidiDataEvent = function (
      myroot,
      playStarting
    ) {
      var midiURL;
  ...
  root.metronomeAutoSpeedUpTempoUpdate = function () {
    ...
    if (document.getElementById("metronomeAutoSpeedupTempoIncreaseAmount"))
      totalTempoIncreaseAmount = parseInt(
        document.getElementById("metronomeAutoSpeedupTempoIncreaseAmount")
          .value,
        10
      );
    ...
    if (document.getElementById("metronomeAutoSpeedupTempoIncreaseInterval")) {
      tempoIncreaseInterval = parseInt(
        document.getElementById("metronomeAutoSpeedupTempoIncreaseInterval")
          .value,
        10
      );
    ...
    var timeDiffMilliseconds =
      totalMidiPlayTime.getTime() -
      class_our_last_midi_tempo_increase_time.getTime();
    var tempoDiffFloat =
      totalTempoIncreaseAmount *
      (timeDiffMilliseconds / (tempoIncreaseInterval * 1000));
    ...
    class_our_last_midi_tempo_increase_remainder =
      tempoDiffFloat - tempoDiffInt;

    class_our_last_midi_tempo_increase_time = totalMidiPlayTime;

    if (!keepIncreasingForever) {
      if (
        curTempo + tempoDiffInt >
        class_our_midi_start_tempo + totalTempoIncreaseAmount
      ) {
        // increase to the total max amount, then we are done
        tempoDiffInt =
          class_our_midi_start_tempo + totalTempoIncreaseAmount - curTempo;
      }
    }
  ...
  function setNotesFromURLData(drumType, noteString, numberOfMeasures) {
    ...
    if (notes.length > notesOnScreen && notes.length / notesOnScreen >= 2) {
      ...
      notes.length < notesOnScreen &&
      notesOnScreen / notes.length >= 2
    ) {
  ...
  function setNotesFromABCArray(drumType, abcArray, numberOfMeasures) {
    ...
    var displayScaler = 1;
    if (
      abcArray.length > notesOnScreen &&
      abcArray.length / notesOnScreen >= 2
    ) {
      // if we encounter a 16th note groove for an 8th note board, let's scale it	down
      noteStringScaler = Math.ceil(abcArray.length / notesOnScreen);
    } else if (
      abcArray.length < notesOnScreen &&
      notesOnScreen / abcArray.length >= 2
    ) {
      // if we encounter a 8th note groove for an 16th note board, let's scale it up
  ...
  function get_FullURLForPage(url_destination) {
    var myGrooveData = root.grooveDataFromClickableUI();
    return root.myGrooveUtils.getUrlStringFromGrooveData(
      myGrooveData,
      url_destination
    );
  }
  ...
  root.setTimeDivisionSelectionOnOrOff = function () {
    ...
    if (
      ((8 * class_num_beats_per_measure) / class_note_value_per_measure) % 1 !=
      0
    ) {
  ...
  root.timeSigPopupClose = function (type, callback) {
    ...
    // ignore type "cancel"
    if (type == "ok") {
      var newTimeSigTop = document.getElementById(
        "timeSigPopupTimeSigTop"
      ).value;
      var newTimeSigBottom = document.getElementById(
  ...
  root.show_FullURLPopup = function () {
        ...
        twitter: {
          before: function () {
            this.url = encodeURIComponent(
              document.getElementById("fullURLPopupTextField").value
            );
            this.description =
    ...
    fillInShortenedURLInFullURLPopup(
      get_FullURLForPage(),
  ...
  function fillInEmbedURLInFullURLPopup(fullURL, cssIdOfTextFieldToFill) {
    ...
    var embedText =
      '<iframe width="100%" height="240" src="' +
      fullURL +
  ...
  root.embedCodeCheckboxChanged = function () {
    if (document.getElementById("embedCodeCheckbox").checked) {
      fillInEmbedURLInFullURLPopup(
        get_FullURLForPage("display"),
        "fullURLPopupTextField"
      );
    } else {
      fillInShortenedURLInFullURLPopup(
  ...
  function set_Default_notes(encodedURLData) {
    ...
    var myGrooveData =
      root.myGrooveUtils.getGrooveDataFromUrlString(encodedURLData);
  ...
  function changeDivisionWithNotes(
    ...
    for (
      var cur_measure = 1;
      cur_measure <= class_number_of_measures;
      cur_measure++
    ) {
    ...
    unselectButton(
      document.getElementById("subdivision_" + oldDivision + "ths")
    );

    // highlight the new div
    selectButton(
      document.getElementById("subdivision_" + class_time_division + "ths")
    );
    ...
    // enable or disable swing
    root.myGrooveUtils.swingEnabled(
      root.myGrooveUtils.doesDivisionSupportSwing(newDivision)
    );
  }

  root.expandAuthoringViewWhenNecessary = function (
    numNotesPerMeasure,
    numberOfMeasures
  ) {
  ...
  root.changeDivision = function (newDivision) {
    ...
    var isNewDivisionTriplets =
      root.myGrooveUtils.isTripletDivision(newDivision);
    var new_notes_per_measure = root.myGrooveUtils.calc_notes_per_measure(
      isNewDivisionTriplets ? 48 : newDivision,
      class_num_beats_per_measure,
      class_note_value_per_measure
    );
    ...
    if (
      ((newDivision * class_num_beats_per_measure) /
        class_note_value_per_measure) %
        1 !=
      0
    ) {
  ...
  root.HTMLforStaffContainer = function (baseindex, indexStartForNotes) {
    ...
    for (
      var i = indexStartForNotes;
      i < class_notes_per_measure + indexStartForNotes;
      i++
    ) {
      newHTML +=
        '\
    ...
    newHTML +=
      '\
										<div class="kick-container">\
											<div class="opening_note_space"> </div> ';
    for (
      var j = indexStartForNotes;
      j < class_notes_per_measure + indexStartForNotes;
      j++
    ) {
      newHTML +=
        '\
    ...
    if (class_number_of_measures > 1)
      ...
      newHTML +=
        '<span class="closeMeasureButton"><i class="fa">&nbsp;&nbsp;&nbsp;</i></span>';
  ...
  root.HTMLforPermutationOptions = function () {
    ...
    var newHTML =
      '<span id="PermutationOptionsHeader">Permutation Options</span>\n';
