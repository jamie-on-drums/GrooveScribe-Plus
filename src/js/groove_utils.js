...
function GrooveUtils() {
  ...
  root.getQueryVariableFromURL = function (variable, def_value) {
    return root.getQueryVariableFromString(
      variable,
      def_value,
  ...
  root.calc_notes_per_measure = function (
    division,
    time_sig_top,
    time_sig_bottom
  ) {
  ...
  root.showContextMenu = function (contextMenu) {
    ...
    if (
      contextMenu.offsetTop + contextMenu.clientHeight >
  ...
  root.GetDefaultTom1Groove = function (
    notes_per_measure,
    timeSigTop,
    timeSigBottom,
    numMeasures
  ...
  root.GetDefaultTom4Groove = function (
    notes_per_measure,
    timeSigTop,
    timeSigBottom,
    numMeasures
  ...
  root.noteArraysFromURLData = function (
    drumType,
    noteString,
    notesPerMeasure,
    numberOfMeasures
    ...
    if (notes.length > retArraySize && notes.length / retArraySize >= 2) {
      ...
      notes.length < retArraySize &&
      retArraySize / notes.length >= 2
    ) {
    ...
    for (
      ...
      retArray[retArrayIndex] = tablatureToABCNotationPerNote(
        drumType,
        notes[j]
      );
    }
  ...
  root.tabLineFromAbcNoteArray = function (
    ...
    for (var i = 0; i < maxLength; i++) {
      ...
      if (drumType == "H" && newTabChar == "X") {
        ...
        if (getAccents && getOthers) returnTabLine += "X"; // kick & splash
        else if (getAccents) returnTabLine += "o"; // just kick
  ...
  root.getGrooveDataFromUrlString = function (encodedURLData) {
    ...
    for (i = 0; i < 4; i++) {
      ...
      var Tom_string = root.getQueryVariableFromString(
        "T" + (i + 1),
        false,
        encodedURLData
      );
    ...
    myGrooveData.title = root.getQueryVariableFromString(
      "title",
      "",
      encodedURLData
    );
    ...
    myGrooveData.author = root.getQueryVariableFromString(
      "author",
      "",
      encodedURLData
    );
    ...
    myGrooveData.comments = root.getQueryVariableFromString(
      "comments",
      "",
      encodedURLData
    );
    ...
    myGrooveData.tempo = parseInt(
      root.getQueryVariableFromString(
        "tempo",
    ...
    if (
      isNaN(myGrooveData.tempo) ||
      myGrooveData.tempo < 20 ||
      myGrooveData.tempo > 400
    )
  ...
  root.getUrlStringFromGrooveData = function (myGrooveData, url_destination) {
    ...
    fullURL +=
      "TimeSig=" + myGrooveData.numBeats + "/" + myGrooveData.noteValue;
    ...
    // notes
    var total_notes =
      myGrooveData.notesPerMeasure * myGrooveData.numberOfMeasures;
    var HH =
      "&H=|" +
  ...
  root.get_top_ABC_BoilerPlate = function (
    ...
    // set the default unit length to the internal full-size unit so
    // durations (scaler * count) map correctly to ABC units
    var fullSizeUnit = root.notesPerMeasureInFullSizeArray(
      isTriplets,
      timeSigTop,
      timeSigBottom
    );
    fullABC += "L:1/" + fullSizeUnit + "\n";
  ...
  function testArrayOfArraysForEquality(
    array_of_arrays,
    test_index,
    test_value
  ) {
  ...
  function getABCforRest(
    ...
    // count the # of rest
    if (
      testArrayOfArraysForEquality(note_array_of_arrays, start_index, false)
    ) {
      var restCount = 1;
      for (
  ...
  root.notesPerMeasureInFullSizeArray = function (
    is_triplet_division,
    timeSigTop,
    timeSigBottom
  ) {
    // a full measure will be defined as 16 * timeSigTop.   (4 = 64, 5 = 80, 6 = 96, etc.)
    // that implies 64th notes in quarter note beats
    // TODO: should we support triplets here?
    if (is_triplet_division) return 48 * (timeSigTop / timeSigBottom);
    else return 64 * (timeSigTop / timeSigBottom);
  };
  ...
  root.getNoteScaler = function (notes_per_measure, timeSigTop, timeSigBottom) {
    ...
    if (!timeSigTop || timeSigTop < 1 || timeSigTop > 36) {
      ...
      if (
        root.isTripletDivisionFromNotesPerMeasure(
          notes_per_measure,
          timeSigTop,
          timeSigBottom
        )
        ...
        scaler = Math.ceil(
          root.notesPerMeasureInFullSizeArray(true, timeSigTop, timeSigBottom) /
            notes_per_measure
        );
      else
        scaler = Math.ceil(
    ...
    for (var i = 0; i < num_notes; i++) {
      ...
      if (
        ...
        if (0 == count_active_notes_in_arrays(all_drum_array_of_array, i, 12)) {
          ...
          sub_division > 12 &&
          0 ==
            count_active_notes_in_arrays(all_drum_array_of_array, i + 1, 5) &&
          ...
          sub_division == 48 &&
          0 ==
            count_active_notes_in_arrays(all_drum_array_of_array, i + 1, 2) &&
          0 ==
            count_active_notes_in_arrays(all_drum_array_of_array, i + 4, 2) &&
          0 ==
            count_active_notes_in_arrays(all_drum_array_of_array, i + 7, 2) &&
  ...
  function snare_HH_kick_ABC_for_quads(
    ...
    // number of positions in a full-size internal array for a non-triplet measure
    var full_notes_per_measure = root.notesPerMeasureInFullSizeArray(
      false,
      timeSigTop,
      timeSigBottom
    );
    ...
    for (var i = 0; i < num_notes; i++) {
      ...
      if (
        (i % notes_per_measure) + grouping_size_for_rests >
        notes_per_measure
      ) {
        // if we are in an odd time signature then the last few notes will have a different grouping to reach the end of the measure
      ...
      // add a bar line every full-size measure
      if ((i + 1) % full_notes_per_measure === 0) {
        stickings_voice_string += "|";
        hh_snare_voice_string += "|";
        kick_voice_string += "|";
      }
      // add a line break every numberOfMeasuresPerLine full-size measures, except the last
      if (
        i < num_notes - 1 &&
        (i + 1) % (full_notes_per_measure * numberOfMeasuresPerLine) === 0
      ) {
        stickings_voice_string += "\n";
        hh_snare_voice_string += "\n";
        kick_voice_string += "\n";
  ...
  root.figure_out_sticking_count_for_index = function (
    ...
    switch (implied_sub_division) {
      ...
      default:
        ...
        if (note_index % 4 === 0)
          new_state = Math.floor(note_index / whole_note_interval) + 1;
  ...
  root.renderABCtoSVG = function (abc_source) {
    ...
    if (
      (root.myGrooveData && root.myGrooveData.showLegend) ||
      root.isLegendVisable
    )
      root.abcNoteNumIndex = -15;
    // subtract out the legend notes for a proper index.
    ...
    if (root.abcNoteNumCurrentlyHighlighted > -1) {
      var myElements = document.querySelectorAll(
        "#abcNoteNum_" +
          root.grooveUtilsUniqueIndex +
          "_" +
          root.abcNoteNumCurrentlyHighlighted
      );
  ...
  root.highlightNoteInABCSVGByIndex = function (noteToHighlight) {
    ...
    for (var i = 0; i < myElements.length; i++) {
      myElements[i].setAttribute(
        "class",
        myElements[i].getAttribute("class") + " highlighted"
      );
  ...
  root.highlightNoteInABCSVGFromPercentComplete = function (percentComplete) {
    if (root.note_mapping_array !== null) {
      ...
      for (
        var i = 0;
        i < curNoteIndex && i < root.note_mapping_array.length;
        i++
      ) {
  ...
  root.midiEventCallbackClass = function (classRoot) {
    ...
    this.playEvent = function (root) {
      var icon = document.getElementById(
        "midiPlayImage" + root.grooveUtilsUniqueIndex
      );
    ...
    this.repeatChangeEvent = function (root, newValue) {
      if (newValue)
        document.getElementById(
          "midiRepeatImage" + root.grooveUtilsUniqueIndex
        ).src = root.getMidiImageLocation() + "repeat.png";
      else
        document.getElementById(
          "midiRepeatImage" + root.grooveUtilsUniqueIndex
        ).src = root.getMidiImageLocation() + "grey_repeat.png";
    };
  ...
  root.MIDI_build_midi_url_count_in_track = function (
    ...
    if (timeSigBottom == 8) noteDelay = 64; // 8th notes over x/8 time
  ...
  root.MIDI_from_HH_Snare_Kick_Arrays = function (
    ...
    if (swing_percentage < 0 || swing_percentage > 0.99) {
      console.log(
        "Swing percentage out of range in GrooveUtils.MIDI_from_HH_Snare_Kick_Arrays"
      );
      swing_percentage = 0;
    }
    ...
    var offsetClickStartBeat =
      root.getMetronomeOptionsOffsetClickStartRotation(isTriplets);
    ...
    for (var i = 0; i < num_notes; i++) {
      ...
      if (metronome_frequency > 0) {
        ...
        switch (offsetClickStartBeat) {
          ...
          case "E":
            if (isTriplets)
              console.log(
                "OffsetClickStart error in MIDI_from_HH_Snare_Kick_Arrays"
              );
        ...
        if (metronome_specific_index >= 0) {
          ...
          if (
            ...
            metronome_specific_index %
              (quarterNoteFrequency * timeSigTop * (4 / timeSigBottom)) ===
              0
          ) {
      ...
      if (!root.metronomeSolo) {
        ...
        switch (HH_Array[i]) {
          ...
          default:
            console.log(
              "Bad case in GrooveUtils.MIDI_from_HH_Snare_Kick_Arrays"
            );
            break;
        }
        ...
        if (Toms_Array) {
          for (
            var which_array = 0;
            which_array < constant_NUMBER_OF_TOMS;
            which_array++
          ) {
            var tom_note = false;
  ...
  root.create_MIDIURLFromGrooveData = function (myGrooveData, MIDI_type) {
    ...
    for (
      var measureIndex = 0;
      measureIndex < myGrooveData.numberOfMeasures;
      measureIndex++
    ) {
      var FullNoteTomsArray = [];
      for (var i = 0; i < constant_NUMBER_OF_TOMS; i++) {
      ...
      root.MIDI_from_HH_Snare_Kick_Arrays(
        midiTrack,
        FullNoteHHArray.slice(
          measure_notes * measureIndex,
          measure_notes * (measureIndex + 1)
        ),
        FullNoteSnareArray.slice(
          measure_notes * measureIndex,
          measure_notes * (measureIndex + 1)
        ),
        FullNoteKickArray.slice(
          measure_notes * measureIndex,
          measure_notes * (measureIndex + 1)
        ),
        FullNoteTomsArray,
  ...
  root.startMIDI_playback = function () {
    if (MIDI.Player.playing) {
      ...
      root.isMIDIPaused &&
      false ===
        root.midiEventCallbacks.doesMidiDataNeedRefresh(
          root.midiEventCallbacks.classRoot
        )
    ) {
      ...
      global_current_midi_start_time = new Date();
      global_last_midi_update_time = 0;
      root.midiEventCallbacks.loadMidiDataEvent(
        root.midiEventCallbacks.classRoot,
        true
      );
      MIDI.Player.stop();
  ...
  root.getMidiPlayTime = function () {
    ...
    var play_time_diff = new Date(
      time_now.getTime() - global_current_midi_start_time.getTime()
    );
    ...
    if (TotalPlayTime) {
      ...
      if (totalTime.getUTCHours() > 0)
        time_string =
          totalTime.getUTCHours() +
          ":" +
          (totalTime.getUTCMinutes() < 10 ? "0" : "");
      time_string +=
  ...
  root.updateMidiPlayTime = function () {
    ...
    var MidiPlayTime = document.getElementById(
      "MIDIPlayTime" + root.grooveUtilsUniqueIndex
    );
  ...
  root.getTempo = function () {
    var tempoInput = document.getElementById(
      "tempoInput" + root.grooveUtilsUniqueIndex
    );
  ...
  root.tempoUpdate = function (tempo) {
    document.getElementById(
      "tempoTextField" + root.grooveUtilsUniqueIndex
    ).value = "" + tempo;
  ...
  root.tempoUpdateFromTextField = function (event) {
    ...
    document.getElementById("tempoInput" + root.grooveUtilsUniqueIndex).value =
      newTempo;
  ...
  root.setSwingSlider = function (newSetting) {
    document.getElementById("swingInput" + root.grooveUtilsUniqueIndex).value =
      newSetting;
  ...
  root.getSwing = function () {
    ...
    if (root.swingIsEnabled) {
      var swingInput = document.getElementById(
        "swingInput" + root.grooveUtilsUniqueIndex
      );
  ...
  root.swingUpdateText = function (swingAmount) {
    if (root.swingIsEnabled === false) {
      document.getElementById(
        "swingOutput" + root.grooveUtilsUniqueIndex
      ).innerHTML = "N/A";
    } else {
      document.getElementById(
        "swingOutput" + root.grooveUtilsUniqueIndex
      ).innerHTML = "" + swingAmount + "%";
  ...
  root.loadFullScreenGrooveScribe = function () {
    var fullURL = root.getUrlStringFromGrooveData(
      root.myGrooveData,
      "fullGrooveScribe"
    );
  ...
  root.expandOrRetractMIDI_playback = function (force, expandElseContract) {
    ...
    if (
      playerControlElement.className.indexOf("small") > -1 ||
      (force && expandElseContract)
    ) {
      // make large
      ...
      midiPlayTime.className =
        midiPlayTime.className.replace(" small", "") + " large";
    } else {
      // make small
      playerControlElement.className =
      ...
      midiPlayTime.className =
        midiPlayTime.className.replace(" large", "") + " small";
    }
  };
  ...
  root.AddMidiPlayerToPage = function (
    HTML_Id_to_attach_to,
    division,
    expandable
  ) {
    ...
    html_element = document.getElementById(
      "tempoInput" + root.grooveUtilsUniqueIndex
    );
    if (html_element) {
      if (isIE10)
        html_element.addEventListener(
          "click",
          root.tempoUpdateFromSlider,
          false
        );
      else
        html_element.addEventListener(
    ...
    if (html_element) {
      html_element.addEventListener(
        "click",
        root.expandOrRetractMIDI_playback,
        false
      );
    }
    ...
    if (html_element) {
      html_element.addEventListener(
        "click",
        root.loadFullScreenGrooveScribe,
        false
      );
    }
